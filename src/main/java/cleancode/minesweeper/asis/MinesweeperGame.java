...
public class MinesweeperGame {
    ...
    public static final String CLONSED_CELL_SIGN = "□";
    ...
    private static void checkIfGameIsOver() { // check는 보통 void를 사용해서 "확인"했다 <-> boolean을 반환하면 isAllCellOpened같은 형태
    ...
    private static boolean isAllCellOpened() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                if (BOARD[row][col].equals(CLONSED_CELL_SIGN)) {
                    isAllOpened = false;
                }
            }
        }
    ...
    private static void initializeGame() {
        for (int row = 0; row < BOARD_ROW_SIZE; row++) { // 단순 i, j가 아닌 row, col로 변경
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                BOARD[row][col] = CLONSED_CELL_SIGN;
            }
        }
